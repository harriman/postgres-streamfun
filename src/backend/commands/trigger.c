...
trigger_call_errcontext(FunctionCallInfo fcinfo)
{
	...
	/* Event type */
	if (TRIGGER_FIRED_BEFORE(trigevent))
		timing = "BEFORE";
	else if (TRIGGER_FIRED_INSTEAD(trigevent))
		timing = "INSTEAD";
	else if (TRIGGER_FIRED_AFTER(trigevent))
		timing = "AFTER";
	else
		timing = "(unknown)";
	...
	if (TRIGGER_FIRED_FOR_ROW(trigevent))
		level = "row";
	else
		level = "statement";
