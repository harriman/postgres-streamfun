...
srf_check_context(FunctionCallInfo fcinfo)
{
	 ...
	 * after a function returns a nonempty result in one mode, it must keep on
	 * using that same mode throughout the query.)
	 */
 	if (rsinfo == NULL ||
 ...
 *
 * If 'required' is true, and the expected result tuple format cannot be
 ...
 * raises an error if 'required' is true, or returns NULL if 'required' is
 * false.  In the latter rare case, the function is free to return tuples in
 * any valid format.
 *
...
srf_get_expected_tupdesc(FunctionCallInfo fcinfo, bool required)
{
	...
	if (rsinfo->expectedDesc == NULL)
	{
		...
		if (required &&	rsinfo->expectedDesc == NULL)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
